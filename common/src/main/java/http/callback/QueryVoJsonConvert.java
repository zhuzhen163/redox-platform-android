...
public class QueryVoJsonConvert<T> implements Converter<T> {
    ...
    public QueryVoJsonConvert(Class<T> clazz) {
        this.clazz = clazz;
    }
    ...
    private T parseParameterizedType(Response response, ParameterizedType type) throws Exception {
        ...
        if (rawType != QueryVoLzyResponse.class) {
            ...
            if (typeArgument == Void.class) {
                ...
                Logger.e("未解密："+content);
                if (!TextUtils.isEmpty(content)) {
                    if (!content.contains("{")) {

                        Logger.e("解密后的数据："+ AesSecret.decode(content));

                        QueryVoLzyResponse lzyResponse = Convert.fromJson(AesSecret.decode(content), type);
                        response.close();
                        return (T) lzyResponse;
                    }
                }
                QueryVoLzyResponse lzyResponse = null;
                try {
                    lzyResponse = Convert.fromJson(content, type);
                } catch (Exception o) {
