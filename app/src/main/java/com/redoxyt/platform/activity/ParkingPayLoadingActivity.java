...
public class ParkingPayLoadingActivity extends BaseActivity{
    ...
    private static class BuildHandle extends Handler {
        private WeakReference<ParkingPayLoadingActivity> reference;//弱引用
        public BuildHandle(ParkingPayLoadingActivity context) {
            reference = new WeakReference<>(context);
        }
        @Override
        public void handleMessage(Message msg) {
            super.handleMessage(msg);
            ParkingPayLoadingActivity activity = reference.get();
    ...
    public void initData() {
        ...
        Glide.with(ParkingPayLoadingActivity.this).asGif().load(R.drawable.yt_pay_loading).into(iv_loading);
    ...
    public void wechatPayRecordPolling() {
        ...
        params.put("attachType","parking_fee_pay");
        OkGo.<QueryVoLzyResponse<PayBean>>get(BaseUrl.YT_Base + BaseUrl.wechatPayRecordPolling)
                .params(params)
                .execute(new QueryVoDialogCallback<QueryVoLzyResponse<PayBean>>(ParkingPayLoadingActivity.this, false) {
                    @Override
                    ...
                    public void onSuccess(com.lzy.okgo.model.Response<QueryVoLzyResponse<PayBean>> response, String desc) {
                        ...
                        if (data.getPayStatus() == 2){
                            EventBus.getDefault().post(new BaseEventMessage("payParking"));
                        }
                        finish();
                    }
                });
