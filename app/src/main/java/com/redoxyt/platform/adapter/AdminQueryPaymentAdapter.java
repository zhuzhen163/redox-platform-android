...
public class AdminQueryPaymentAdapter extends ListBaseAdapter <ReservationBean>{

    private TextView tv_reserveCode,tv_reserveStartDate,tv_carCode,tv_warehouseName,tv_driverName,
            tv_transportCode,tv_driverMobile,tv_platformName,tv_payTime,tv_payAmount,tv_cancelPay,tv_pay;

    private OnItemClickListener mOnItemClickListener;

    public AdminQueryPaymentAdapter(Context context) {
        super(context);
    }
    ...
    public int getLayoutId() {
        return R.layout.adapter_admin_query_payment;
    }
    ...
    public void onBindItemHolder(SuperViewHolder holder, int position) {
        ...
        tv_cancelPay = holder.getView(R.id.tv_cancelPay);
        ...
        tv_cancelPay.setVisibility(View.GONE);
        tv_pay.setVisibility(View.GONE);
        int reserveStatus = bean.getReserveStatus();
        if (reserveStatus<4){
            tv_cancelPay.setVisibility(View.VISIBLE);
        }
        if (reserveStatus==0 && bean.getPayAmount()>0){
            tv_pay.setVisibility(View.VISIBLE);
        }

        tv_pay.setOnClickListener(view -> {
            if (mOnItemClickListener != null){
                mOnItemClickListener.onItemPayment(bean.getReserveId(),bean.getWarehouseGroupId());
            }
        });

        tv_cancelPay.setOnClickListener(view -> {
            if (mOnItemClickListener != null){
                mOnItemClickListener.onItemCancelPay(bean.getReserveId());
            }
        });
    ...
    public interface OnItemClickListener {
        void onItemPayment(String reserveId,String warehouseGroupId);
        void onItemCancelPay(String reserveId);
    }
